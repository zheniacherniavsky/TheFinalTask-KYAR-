...
function math() {
	...
	document.write(76 + '0<br>');
	document.write('Результатом сложения числа и строки всегда будет - строка.');
}
function thirdtask() {
...
function sixthtask() {
	...
	if (day == 7) nameOfDay = 7; // Потому что остаток
	// от делеояни 7 на 7 равен 0

	if (day > 31) {
		alert('Введите конкретное значение');
		break;
	}
...
function seventask() {
	try {
		...
		alert("Ваша функция не работает!!!!");
		console.log(e);
	}
}